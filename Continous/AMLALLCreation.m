...
GeneNum = 7129;
%% Load Train
fileTrain='DIRECTORY IN YOUR COMPUTER/AMLALL_train.data';
...
fileTest='DIRECTORY IN YOUR COMPUTER/AMLALL_test.data';
format = [repmat('%f,', [1 GeneNum]) , '%s'];
fid = fopen(fileTest);
x = textscan(fid, format);
fclose(fid);
...
[NormalizeDs] = Normalize01(AMLALLnonzero);
Ds=[NormalizeDs,Label];
